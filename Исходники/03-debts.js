...
function loanSchedule(a){
  ...
  if(a.scheduleMode==='manual' && Array.isArray(a.manualSchedule) && a.manualSchedule.length){
    let bal = Number(a.openingBalance)||0;
    const used = new Set();
    /* Недоплата с прошлой строки — прибавляется к следующей: заплатили
       меньше в этом месяце, значит именно на эту разницу вырастет
       следующий платёж, а не просто «остаток» где-то в стороне. */
    let carry = 0;
    for(const r of [...a.manualSchedule].sort((x,y)=>x.date.localeCompare(y.date))){
      ...
      const scheduled = Number(r.amount)||0;
      const due = round2(scheduled + carry);
      const key = planKey('debt', a.id, r.date);
      const ov = planOverride(key);
      /* Свой платёж на этот месяц — только пока платёж ещё не внесён фактически. */
      const overridden = !!(!paid && ov && ov.amount != null);
      const amount = overridden ? (Number(ov.amount) || 0) : due;
      const bumped = !overridden && carry > 0.01;
      const interest = r.interest!=null ? Number(r.interest) : round2(bal*rate);
      const actual = paid ? paid.amount : amount;
      const principal = round2(actual - interest);
      bal = round2(Math.max(0, bal - principal));
      carry = paid ? 0 : round2(Math.max(0, due - amount));
      rows.push({date:r.date, payment: paid ? paid.amount : amount, interest, principal, balance:bal,
                 paid: !!paid, paidAmount: paid?paid.amount:null, paidDate: paid?paid.date:null,
                 overridden, bumped, planKey: key});
    }
    /* Список банка кончился, а долг остался — например, где-то внесли меньше
       положенного. Молча терять эти деньги нельзя: продолжаем график тем же
       платежом, что был в последней строке банка, пока долг не закроется. */
    if(bal > 0.01 && a.manualSchedule.length){
      const lastRow = [...a.manualSchedule].sort((x,y)=>x.date.localeCompare(y.date)).slice(-1)[0];
      let d = addMonths(lastRow.date, 1);
      const contPay = Number(a.payment) || Number(lastRow.amount) || 0;
      let guard = 0;
      while(bal > 0.01 && contPay > 0 && guard < 600){
        const interest = round2(bal*rate);
        ...
        if(pay <= interest && rate>0){
          rows.push({date:d, payment:pay, interest, principal:0, balance:bal, error:true, overridden, planKey:key, continued:true});
          break;
        }
        let principal = round2(pay - interest);
        if(principal >= bal){ principal = bal; pay = round2(bal + interest); }
        bal = round2(bal - principal);
        rows.push({date:d, payment:pay, interest, principal, balance:bal, paid:false, overridden, planKey:key, continued:true});
        d = addMonths(d, 1);
        guard++;
      }
    }
  ...
  let guard = 0;
  /* Недоплата с прошлой строки — прибавляется к следующей, платёж растёт
     ровно на разницу, а не просто «где-то в остатке». */
  let carry = 0;
  while(bal > 0.01 && guard < 600){
    const interest = round2(bal*rate);
    const due = round2(payment + carry);
    /* Свой платёж на этот месяц — например, если получается внести только
       минимум. Недостающая часть перейдёт в следующий платёж. */
    const key = planKey('debt', a.id, d);
    const ov = planOverride(key);
    const overridden = !!(ov && ov.amount != null);
    let pay = overridden ? (Number(ov.amount) || 0) : due;
    const bumped = !overridden && carry > 0.01;
    if(pay <= interest && rate>0){ // платёж не покрывает проценты — некорректные данные
      rows.push({date:d, payment:pay, interest, principal:0, balance:bal, error:true, overridden, bumped, planKey:key});
      break;
    }
    let principal = round2(pay - interest);
    if(principal >= bal){ principal = bal; pay = round2(bal + interest); carry = 0; }
    else carry = round2(Math.max(0, due - pay));
    bal = round2(bal - principal);
    rows.push({date:d, payment:pay, interest, principal, balance:bal, paid:false, overridden, bumped, planKey:key});
    d = addMonths(d, 1);
    guard++;
  }
  return rows;
...
function cardSchedule(a, mode){
  ...
  if(a.scheduleMode === 'manual' && Array.isArray(a.manualSchedule) && a.manualSchedule.length){
    ...
    for(const r of [...a.manualSchedule].sort((x,y)=>x.date.localeCompare(y.date))){
      ...
      carry = paid ? 0 : round2(Math.max(0, due - amount));
      rows.push({date:r.date, payment: paid ? paid.amount : amount, interest, principal, balance:left,
                 paid: !!paid, paidAmount: paid?paid.amount:null,
                 overridden, bumped, planKey: key});
    }

    /* Список банка кончился, а долг остался — например, где-то внесли меньше
       минимального. Продолжаем графиком минимального платежа по формуле
       карты (% от остатка), пока долг не закроется. */
    if(left > 0.01 && a.manualSchedule.length){
      const lastRow = [...a.manualSchedule].sort((x,y)=>x.date.localeCompare(y.date)).slice(-1)[0];
      let d = addMonths(lastRow.date, 1);
      let guard = 0;
      while(left > 0.01 && guard < 720){
        const inGrace = !!(gEnd && d <= gEnd);
        const interest = inGrace ? 0 : round2(left*rate);
        let pay = cardMinPayment(a, left);
        const key = planKey('debt', a.id, d);
        const ov = planOverride(key);
        const overridden = !!(ov && ov.amount != null);
        if(overridden) pay = Number(ov.amount) || 0;
        if(pay <= interest){
          rows.push({date:d, payment:pay, interest, principal:0, balance:left, error:true, overridden, planKey:key, continued:true});
          break;
        }
        let principal = round2(pay - interest);
        if(principal >= left){ principal = left; pay = round2(left + interest); }
        left = round2(left - principal);
        rows.push({date:d, payment:pay, interest, principal, balance:left, paid:false, grace:inGrace, overridden, planKey:key, continued:true});
        d = addMonths(d, 1);
        guard++;
      }
    }
...
function installmentSchedule(a){
  ...
  /* Дату ближайшего платежа берём как есть, даже если она уже в прошлом
     (например, 17-е указали, а сегодня 24-е) — иначе число месяца съезжает
     на сегодняшний день и весь график идёт вразнобой. Совпадает с тем,
     как считается график по кредиту. */
  let d = a.nextPaymentDate || today();
  let guard = 0;
  /* Недоплата с прошлой строки — прибавляется к следующей. */
  let carry = 0;
  while(bal > 0.01 && guard < 240){
    const due = round2(pay + carry);
    const key = planKey('debt', a.id, d);
    const ov = planOverride(key);
    const overridden = !!(ov && ov.amount != null);
    const wanted = overridden ? (Number(ov.amount)||0) : due;
    const bumped = !overridden && carry > 0.01;
    const amount = Math.min(wanted, bal);
    carry = round2(Math.max(0, due - amount));
    bal = round2(bal - amount);
    rows.push({date:d, payment:amount, interest:0, principal:amount, balance:bal, paid:false, overridden, bumped, planKey:key});
...
function showSchedule(id){
  ...
  const manualBlock = (a.type==='loan' || a.type==='credit_card') ? `
    ...
    <div class="note">Если банк присылает точный график — внесите его, и расчёт пойдёт по нему.
      Можно вписать строки вручную или распознать скриншот из приложения банка.
      Заплатите в какой-то месяц меньше — недостающая часть прибавится к следующему
      платежу (помечен «с переносом»), а если строк банка не хватит, чтобы её
      закрыть, график продолжится минимальным платежом по формуле («по расчёту»).</div>
...
function schedRow(r, accountId){
  ...
  const mark = r.error ? '<span class="chip bad">платёж меньше процентов</span>'
             ...
             : r.bumped ? '<span class="chip info">с переносом</span>'
             : r.continued ? '<span class="chip info">по расчёту</span>'
...
function editScheduleRow(accountId, key, date, amount){
  ...
  document.getElementById('ovSchedBody').innerHTML = `
    ...
    <div class="note">Не получается внести всю сумму по графику — впишите сколько
      ...
      а недостающая часть прибавится к следующему платежу — он вырастет ровно
      на неё. Более поздние платежи не изменятся.</div>
