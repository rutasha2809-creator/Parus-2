...
function forecastMonths(F){
  ...
  /* Дополняем первый месяц фактическими операциями за прошедшие дни
     (до today), которые не попали в прогноз. Без этого текущий месяц
     показывает только остаток периода и занижает доходы/расходы. */
  if(out.length && F.length){
    const t0 = F[0].date;
    const mStart = t0.slice(0,8) + '01';
    if(mStart < t0){
      const past = txInRange(mStart, addDays(t0, -1));
      const m0 = out[0];
      for(const t of past){
        if(t.type === 'transfer') continue;
        const v = txBase(t);
        if(t.type === 'income') m0.inc += v;
        else m0.exp += v;
        m0.moves++;
      }
    }
