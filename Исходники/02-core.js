...
/* Допуск при сопоставлении плана с фактом: банк проводит платёж не день в день */
const MATCH_DAYS = 4;
...
function openTx(id, presetAccountId){
  ...
  /* Операция уже отмечена регулярной — показываем это состояние,
     иначе галочка выглядела бы снятой и правка терялась. */
  const linked = t ? recForTx(t.id) : null;
  if(linked){
    const cb = document.getElementById('txRepeat');
    if(cb) cb.checked = true;
    const f1 = document.getElementById('txRepeatFreq');
    if(f1 && linked.freq) f1.value = linked.freq;
    const f2 = document.getElementById('txRepeatUntil');
    if(f2 && linked.endDate) f2.value = linked.endDate;
    const f3 = document.getElementById('txRepeatShift');
    if(f3) f3.checked = !!linked.shiftWeekend;
    txRepeatToggle();
  }
  openOv('ovTx');
}

/* Повторяющееся начисление, заведённое из этой операции */
...
function saveTx(id){
  ...
  /* Отмечена галочка «регулярный» — заводим повторяющееся начисление.
     ...
     Работает и при правке уже сохранённой операции: галочку можно
     поставить позже, а изменённые сумма и дата подхватятся повтором. */
  let repeated = null, dropped = false;
  const repEl = document.getElementById('txRepeat');
  const wantRepeat = repEl && repEl.checked && TXKIND !== 'transfer';
  const linked = recForTx(rec.id);
  ...
  if(wantRepeat){
    ...
    if(!until || startDate <= until){
      ...
      if(linked){ repeated = Object.assign(linked, body); }
      else { repeated = Object.assign({ id: uid() }, body); S.recurring.push(repeated); }
    } else if(linked){
      S.recurring = S.recurring.filter(r => r !== linked); dropped = true;
    }
  } else if(linked){
    /* галочку сняли вручную — повтор убираем, сама операция остаётся */
    S.recurring = S.recurring.filter(r => r !== linked); dropped = true;
  }
...
function deleteTx(id){
  ...
  /* Осиротевший повтор продолжал бы наполнять календарь без видимой причины */
  const linked = recForTx(id);
  if(linked && confirm('У этой операции был регулярный повтор «' + linked.name + '».\nУбрать его тоже?')){
    S.recurring = S.recurring.filter(r => r !== linked);
  }
...
async function wipeData(){
  ...
  /* Если включена облачная синхронизация — очистку нужно отправить и туда.
     Иначе при следующем входе приложение решит, что локально «просто ничего
     не меняли», и молча подтянет старые данные обратно из облака. */
  if(typeof sb !== 'undefined' && sb && typeof sbUser !== 'undefined' && sbUser){
    try{
      await syncPush();
      toast('Данные очищены — на этом устройстве и в облаке');
    }catch(e){
      console.error('wipeData: не удалось отправить очистку в облако', e);
      toast('Данные очищены локально, но не в облаке — нажмите «Очистить всё» ещё раз, когда будет интернет');
    }
  } else {
